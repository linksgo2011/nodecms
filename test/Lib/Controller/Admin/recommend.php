...
class Recommend extends CI_Controller {
	var $tablefunc = 'recommend';
	var $fields = array('title','model','remark','updatetime','listorder','status');
	var $funcarr = array('add','order','del');
	var $editlang,$modelarr;
	function __construct(){
		...
		$this->modelarr = mult_to_single($this->Data_model->getData(array('status'=>1,'isrecommend'=>1),'listorder',0,0,'model'),'varname');
	}
	...
	function _setlist($data,$ismultiple=true){
		...
		foreach($newdata as $key=>$item){
			...
			<td width=150>'.lang('model_'.$item['model']).'</td>
			<td>'.$item['remark'].'</td>
