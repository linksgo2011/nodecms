...
class Link extends CI_Controller {
	var $tablefunc = 'link';
	var $fields = array('type','title','description','thumb','url','remark','listorder','status');
	...
	function _setlist($data,$ismultiple=true){
		...
		foreach($newdata as $key=>$item){
			...
			<td width=200>'.$typestr.'<a href="'.$item['url'].'" target="_blank">'.$item['title'].'</a></td>
			<td width=200 style="word-break:break-all;"><a href="'.$item['url'].'" target="_blank">'.$item['url'].'</a></td>
