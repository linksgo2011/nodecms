<?php if($tpl=='list'):?>
	...
	<option value="<?=$type?>"<?php if ($search['type'] ==$type): ?>selected<?php endif; ?>><?=lang('online_'.$type)?></option>
	<?php endforeach;?>
	</select>
	...
	<tr>
		...
		<th width=200  align=left><?=lang('type')?></th>
		<th align="left"><?=lang('online_num_code')?></th>
...
<?php elseif($tpl=='view'):?>
	...
	<tr><td width="100"><?=lang('type')?></td>
	<td><select name="type"  id="type" class="validate" validtip="required"><option value=""><?=lang('type_pselect')?></option>
	<?php foreach($typearr as $type):?>
	<option value="<?=$type?>"<?php if (isset($view['type'])&&$view['type'] ==$type): ?>selected<?php endif; ?>><?=lang('online_'.$type)?></option>
	<?php endforeach;?>
	</select></td>
	</tr>
	...
	<tr><td width="100"><?=lang('online_num_code')?></td><td><textarea name="description" id="description" class="txtarea" cols="40" rows="5"><?=isset($view['description'])?$view['description']:'';?></textarea></td></tr>
