...
class Guestbook extends CI_Controller {
	var $tablefunc = 'guestbook';
	var $fields = array('category','title','author','email','phone','description','content','listorder','status');
	...
	function _setlist($data,$ismultiple=true){
		...
		foreach($newdata as $key=>$item){
			...
			<td>'.$categorystr.$item['title'].'</td>
			<td width=80>'.$item['author'].'</td>
