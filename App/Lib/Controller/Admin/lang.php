...
class Lang extends CI_Controller {
	var $tablefunc = 'lang';
	var $fields = array('title','varname','icon','listorder','status');
	...
	public function index(){
		...
		$pagearr=array(
			...
			'totalnum'=>$this->Data_model->getDataNum(),
			'pagenum'=>20
		);
	...
	function _setlist($data,$ismultiple=true){
		...
		foreach($newdata as $key=>$item){
			...
			$langicon = $item['icon']?'<img src="'.get_image_url($item['icon']).'" />':'';
